...
from qudi.util.linear_transform import (
    compute_rotation_matrix_to_plane,
...
class SpadProbeLogic(LogicBase):
    ...
    _camera = Connector(name="spad", interface=CameraInterface)
    ...
    def on_activate(self):
        ...
        try:
            ...
            self.log.warning(
                "Scan settings in Status Variable invalid, using defaults.", exc_info=e
            )
            self.set_default_scan_settings()
    ...
    def set_back_scan_settings(self, setting: ScanSettings) -> None:
        ...
        if not self.use_back_scan_settings:
            ...
            self.log.info(
                f"Tried to set a back scan setting without back scans turned on."
    ...
    def check_scan_settings(self):
        ...
        for stg in [self.scan_ranges, self.scan_resolution, self.scan_frequency]:
            ...
            for ax in axs:
                if ax not in self.scanner_axes.keys():
                    self.log.debug(
                        f"Axis {ax} from scan settings not available on scanner"
                    )
                    raise ValueError
        ...
        for dim in [1, 2]:
            for axes in combinations(self.scanner_axes, dim):
                ...
                back_settings = self.create_back_scan_settings(axes)
                self.scanner_constraints.check_back_scan_settings(
    ...
    def set_back_scan_resolution(self, axis: str, resolution: int) -> None:
        with self._thread_lock:
            if self.module_state() != "idle":
                self.log.warning(
                    "Scan is running. Unable to change back scan resolution."
                )
            elif (
                BackScanCapability.RESOLUTION_CONFIGURABLE
                not in self.back_scan_capability
            ):
                ...
                if resolution != self.scan_resolution[axis] and resolution != 0:
                    self.log.error(
                        "Backward scan resolution must be the same as forward resolution for this scanner."
                    )
            else:
                ...
                try:
                    ...
                    self.scanner_constraints.check_back_scan_settings(
                        back_settings, forward_settings
                    )
                except Exception as e:
    ...
    def set_back_scan_frequency(self, axis: str, frequency: float) -> None:
        with self._thread_lock:
            if self.module_state() != "idle":
                self.log.warning(
                    "Scan is running. Unable to change back scan frequency."
                )
            elif (
                ...
                if frequency != self.scan_frequency[axis] and frequency != 0.0:
                    self.log.error(
                        "Backward scan frequency must be the same as forward frequency for this scanner."
                    )
            else:
    ...
    def set_target_position(self, pos_dict, caller_id=None, move_blocking=False):
        with self._thread_lock:
            if self.module_state() != "idle":
                self.log.error(
                    "Unable to change scanner target position while a scan is running."
                )
    ...
    def configure_tilt_correction(self, support_vecs=None, shift_vec=None):
        ...
        support_vecs_arr = np.asarray(
            self.tilt_vector_dict_2_array(support_vecs, reduced_dim=False)
        )
        if shift_vec is not None:
            shift_vec_arr = np.array(
                self.tilt_vector_dict_2_array(shift_vec, reduced_dim=False)
            )

        if support_vecs_arr.shape[0] != 3:
            raise ValueError(
                f"Need 3 n-dim support vectors, not {support_vecs_arr.shape[0]}"
            )
        ...
        rot_mat = compute_rotation_matrix_to_plane(
            red_support_vecs[0], red_support_vecs[1], red_support_vecs[2]
        )
        shift = shift_vec_arr
        ...
        self._tilt_corr_axes = [
            el for idx, el in enumerate(self._scan_axes) if tilt_axes[idx]
        ]
        ...
        with self._thread_lock:
            if self.module_state() != "idle":
                self.sigScanStateChanged.emit(
                    True, self.scan_data, self.back_scan_data, self._curr_caller_id
                )
                return
            ...
            try:
                ...
                self.log.error(
                    "Could not set scan settings on scanning probe hardware.",
                    exc_info=e,
                )
                return
            ...
            self.log.debug(
                f"Successfully configured scanner and logic scan poll timer: {t_poll_ms} ms"
            )
    ...
    def stop_scan(self):
        with self._thread_lock:
            if self.module_state() == "idle":
                self.sigScanStateChanged.emit(
                    False, self.scan_data, self.back_scan_data, self._curr_caller_id
                )
                return
            ...
            try:
                ...
                if self.save_to_history:
                    self.sigNewScanDataForHistory.emit(
                        self.scan_data, self.back_scan_data
                    )
    ...
    def set_default_scan_settings(self):
        ...
        self._scan_resolution = {
            ax: axes[ax].resolution.default for ax in self.scanner_axes
        }
        self._scan_frequency = {
            ax: axes[ax].frequency.default for ax in self.scanner_axes
        }
    ...
    def __transform_func(self, coord, inverse=False):
        ...
        coord_reduced = {
            key: val
            for key, val in list(coord.items())[:3]
            if key in self._tilt_corr_axes
        }
        ...
        [
            coord_transf.update({ax: coord_vec_transf[idx]})
            for (idx, ax) in enumerate(self._tilt_corr_axes)
        ]
