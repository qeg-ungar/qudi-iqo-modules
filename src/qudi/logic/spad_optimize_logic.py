...
class ScanningOptimizeLogic(LogicBase):
    ...
    def _scan_state_changed(
        ...
        back_scan_data: Optional[ScanData],
        caller_id: UUID,
    ):
        with self._thread_lock:
            if (
                ...
                try:
                    if data.settings.scan_dimension == 1:
                        x = np.linspace(
                            *data.settings.range[0], data.settings.resolution[0]
                        )
                        ...
                        y = np.linspace(
                            *data.settings.range[1], data.settings.resolution[1]
                        )
