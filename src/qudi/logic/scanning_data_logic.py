...
class ScanningDataLogic(LogicBase):
    ...
    _scan_history: List[Tuple[ScanData, Optional[ScanData]]] = StatusVar(
        name="scan_history", default=list()
    )
    ...
    def __scan_history_to_dicts(
        self, history: List[Tuple[ScanData, Optional[ScanData]]]
    ...
    def __scan_history_from_dicts(
        self, history_dicts: List[List[Optional[Dict]]]
        ...
        try:
            for data_dict, back_data_dict in history_dicts:
                ...
                back_data = (
                    ScanData.from_dict(back_data_dict)
                    if back_data_dict is not None
                    else None
                )
            ...
            self.log.warning(
                "Unable to load scan history. Deleting scan history.", exc_info=e
            )

        if data_dropped:
            self.log.warning(
                "Deleted scan history entries containing an incompatible scan axes configuration."
            )

        return history

    def get_last_history_entry(
        self, scan_axes: Optional[Tuple[str, ...]] = None
    ...
    def restore_from_history(
        self, scan_axes: Optional[Tuple[str, ...]] = None, set_target: bool = False
    ):  # 20260112 https://github.com/Ulm-IQO/qudi-iqo-modules/issues/214#issuecomment-3738324832
    ...
    def history_previous(self):
        with self._thread_lock:
            if self._curr_history_index < 1:
                self.log.warning(
                    "Unable to restore previous state from scan history. "
    ...
    def history_next(self):
        with self._thread_lock:
            if self._curr_history_index >= len(self._scan_history) - 1:
                self.log.warning(
                    "Unable to restore next state from scan history. "
                    "Already at latest history entry."
                )
                return
            return self._restore_from_history_index(self._curr_history_index + 1)

    def _restore_from_history_index(
        self, index: int, set_target: bool = False
    ):  # 20260112 https://github.com/Ulm-IQO/qudi-iqo-modules/issues/214#issuecomment-3738324832
        with self._thread_lock:
            ...
            try:
                ...
                self.log.exception(
                    'Unable to restore scan history with index "{0}"'.format(index)
                )
                return
    ...
    def _get_last_history_entry_index(
        self, scan_axes: Optional[Tuple[str, ...]] = None
    ) -> Union[int, None]:
        """
    ...
    def draw_1d_scan_figure(self, scan_data, channel):
        ...
        xy_plot = ax.plot(x_axis / si_factor_x, data / si_factor_data)
    ...
    def save_scan(
        self, scan_data, color_range=None, is_back: bool = False, custom_tag: str = None
    ):
        with self._thread_lock:
            ...
            try:
                ...
                parameters = {}
                for range, resolution, unit, axis in zip(
                ...
                parameters[f"scanner target at start"] = (
                    scan_data.scanner_target_at_start
                )
                ...
                if scan_data.scanner_target_at_start:
                    for new_ax in scan_data.scanner_target_at_start.keys():
                        if new_ax not in scan_data.settings.axes:
                            ax_info = self._scan_logic().scanner_constraints.axes[
                                new_ax
                            ]
                ...
                for channel, data in scan_data.data.items():
                    ...
                    tag = self.create_tag_from_scan_data(
                        scan_data, channel, is_back, custom_tag
                    )
                    ...
                    if len(scan_data.settings.axes) == 1:
                        ...
                        figure = self.draw_2d_scan_figure(
                            scan_image, cbar_range=color_range
                        )
                        ax = plt.gca()
                        ...
                        self.log.warning(
                            "No figure saved for data with more than 2 dimensions."
                        )
    ...
    @staticmethod
    def create_tag_from_scan_data(
        scan_data: ScanData, channel: str, is_back: bool = False, custom_tag: str = None
    ):
    ...
    def draw_2d_scan_figure(self, scan_image: ScanImage, cbar_range=None):
        ...
        si_prefix_cb = (
            ScaledFloat(cbar_range[1] - cbar_range[0]).scale
        ...
        # Create image plot
        cfimage = ax.imshow(
            image_arr.transpose() / si_factor_cb,
            ...
            interpolation="none",
            extent=(
                *np.asarray(scan_image.ranges[0]) / si_factor_x,
        ...
        ax.set_aspect(1)
        ax.set_xlabel(
            scan_axes[0] + f" position ({si_prefix_x}{scan_image.axis_units[0]})"
        )
        ax.set_ylabel(
            scan_axes[1] + f" position ({si_prefix_y}{scan_image.axis_units[1]})"
        )
        ax.spines["bottom"].set_position(("outward", 10))
        ax.spines["left"].set_position(("outward", 10))
        ax.spines["top"].set_visible(False)
        ...
        if scan_image.data_unit:
            cbar.set_label(
                f"{scan_image.data_name} ({si_prefix_cb}{scan_image.data_unit})"
            )
        else:
    ...
    def _add_draw_scanner_pos(self, ax, scan_data: ScanData):
        ...
        if np.min(scan_range_x) < pos_x < np.max(scan_range_x) and np.min(
            scan_range_y
        ) < pos_y < np.max(scan_range_y):
            trans_xmark = mpl.transforms.blended_transform_factory(
                ax.transData, ax.transAxes
            )
