...
class LaserLogic(LogicBase):
    ...
    def on_activate(self):
        ...
        self._last_trigger_mode = laser.get_trigger_mode()
    ...
    def trigger_mode(self):
        with self._thread_lock:
            self._last_trigger_mode = self._laser().get_trigger_mode()
            return self._last_trigger_mode
        ...
        with self._thread_lock:
            ...
            try:
                ...
                if current_setpoint != self._last_current_setpoint:
                    ...
                    self.sigCurrentSetpointChanged.emit(
                        self._last_current_setpoint, None
                    )
            except:
                pass
            try:
                ...
                pass
            try:
                trigger_mode = laser.get_trigger_mode()
                if trigger_mode != self._last_trigger_mode:
                    self._last_trigger_mode = trigger_mode
                    self.sigTriggerModeChanged.emit(self._last_trigger_mode, None)
            except:
                pass
            ...
            try:
                ...
                self.log.exception(
                    "Exception in laser data query. Stopping laser polling."
                )
            else:
    ...
    def start_query_loop(self):
        ...
        if self.thread() is not QtCore.QThread.currentThread():
            QtCore.QMetaObject.invokeMethod(
                self, "start_query_loop", QtCore.Qt.BlockingQueuedConnection
            )
            return
    ...
    def stop_query_loop(self):
        ...
        if self.thread() is not QtCore.QThread.currentThread():
            QtCore.QMetaObject.invokeMethod(
                self, "stop_query_loop", QtCore.Qt.BlockingQueuedConnection
            )
            return
    ...
    def set_trigger_mode(self, mode):
        """Change laser trigger mode (LAS/EXT/...)."""
        with self._thread_lock:
            try:
                mode = TriggerMode(mode)
            except ValueError:
                self.log.error(f'Invalid trigger mode to set: "{mode}"')
            else:
                try:
                    self._laser().set_trigger_mode(mode)
                except:
                    self.log.exception("Error while setting laser trigger mode:")
            self.sigTriggerModeChanged.emit(self.trigger_mode)
    ...
    def _initialize_data(self):
        self._data = {
            name: np.zeros(self._buffer_length)
            for name in self._laser().get_temperatures()
        }
        self._data["time"] = (
            time.time() - np.arange(self._buffer_length)[::-1] * self._query_interval
        )
