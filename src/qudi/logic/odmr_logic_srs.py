...
class OdmrLogic(LogicBase):
    ...
    _data_scanner = Connector(
        name="data_scanner", interface="FiniteSamplingInputInterface"
    )
    ...
    _scan_frequency_ranges = StatusVar(
        name="scan_frequency_ranges", default=[(2820e6, 2920e6, 101)]
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._raw_linedata = None  # added by shg 2024 03
        self._raw_data = None
    ...
    def on_activate(self):
        ...
        self._run_time = max(1.0, self._run_time)
    ...
    def _initialize_odmr_data(self):
        ...
        self._raw_linedata = dict()  # added by shg 2024 03
        self._raw_data = dict()
        self._fit_results = dict()
        self._signal_data = dict()
        estimated_samples = self._run_time * self._data_rate
        samples_per_line = sum(
            freq_range[-1] for freq_range in self._scan_frequency_ranges
        )
        # Add 5% Safety; Minimum of 1 line
        self.__estimated_lines = max(
            1, int(1.05 * estimated_samples / samples_per_line)
        )
        for channel in self._data_scanner().constraints.channel_names:
            self._raw_data[channel] = [
                np.full((freq_arr.size, self.__estimated_lines), np.nan)
                for freq_arr in self._frequency_data
            ]
            ...
            #
            self._raw_linedata[channel] = np.zeros(
                len(np.concatenate(self._frequency_data))
            )  # added by shg 2024 03
            ...
            for range_index, raw_data in enumerate(raw_data_list):
                ...
                if masked_raw_data.compressed().size == 0:
                    ...
                    self._signal_data[channel][range_index] = np.mean(
                        masked_raw_data[:, : self._scans_to_average], axis=1
                    ).compressed()
    ...
    def set_frequency_range_count(self, number_of_ranges):
        ...
        with self._threadlock:
            if self.module_state() != "idle":
                self.log.error(
                    "Unable to set frequency range count. ODMR scan in progress."
                )
                self.sigScanParametersUpdated.emit(
                    {"frequency_ranges": self.frequency_ranges}
                )
            ...
            if number_diff < 0:
                ...
                if constraints.mode_supported(SamplingOutputMode.JUMP_LIST):
                    ...
                    self.log.error(
                        "Multiple frequency ranges not supported by ODMR scanner "
    ...
    def set_sample_rate(self, data_rate=None, oversampling=None):
        ...
        with self._threadlock:
            ...
            if self.module_state() == "locked":
                self.log.error(
                    "Unable to set sample rate. ODMR measurement in progress."
                )
            else:
                ...
                oversampling = (
                    ...
                    else max(1, int(oversampling))
                )
                if self.data_constraints.sample_rate_in_range(data_rate * oversampling)[
                    0
                ]:
                    ...
                    self.log.error(
                        "Unable to set sample rate. Resulting sample rate out of bounds "
    ...
    def toggle_cw_output(self, enable):
        with self._threadlock:
            ...
            if enable and self.module_state() != "idle":
                self.log.error(
                    "Unable to turn on microwave CW output. ODMR scan in progress."
                )
                return
    ...
    def start_odmr_scan(self):
        ...
        with self._threadlock:
            if self.module_state() != "idle":
                self.log.error(
                    "Can not start ODMR scan. Measurement is already running."
                )
            ...
            try:
                ...
                if (
                    self._default_scan_mode != SamplingOutputMode.JUMP_LIST
                    and len(self._scan_frequency_ranges) > 1
                ):
                    mode = SamplingOutputMode.JUMP_LIST
                    self.log.info(
                        "Multiple ODMR scan ranges set up. Trying to switch scanner to "
                ...
                if mode == SamplingOutputMode.JUMP_LIST:
                    ...
                    #):  # commented by shg 2024 03 #uncommented by aws&aeu 7/18 #ununcommented 8/1 for sampling factor fix
                    #    frequencies = np.repeat(
                    #        frequencies, self._oversampling_factor
                    #    )  # commented by shg 2024 03
                ...
                sampler.set_frame_size(
                    samples
                )  # commented by shg 2024 03 # return to old way 7/18
                # sampler.set_frame_size(self._oversampling_factor)  # added by shg 2024 03
                # print(sample_rate,self._oversampling_factor)

                # Set up microwave scan and start it
                microwave.configure_scan(
                    self._scan_power, frequencies, mode, sample_rate
                )  # #shg 2024 03 microwave._scan_sample_rate
                microwave.start_scan()
                # time.sleep(5)
            except:
                self.module_state.unlock()
                self.log.exception(
                    "Unable to start ODMR scan. Error while setting up hardware:"
                )
                self.sigScanStateUpdated.emit(False)
                return

            #### Added for MUQUABIS
            print("sending 1st trigger")
            microwave._write("*TRG")  # use the nicer function with error handling here
            ####
        ...
        for channel, range_data in self._raw_data.items():
            for range_index, _ in enumerate(range_data):
                ...
                self.sigFitUpdated.emit(
                    self._fit_results[channel][range_index], channel, range_index
                )
    ...
    def continue_odmr_scan(self):
        ...
        with self._threadlock:
            if self.module_state() == "locked":
                self.log.error(
                    "Can not continue ODMR scan. Measurement is already running."
                )
                self.sigScanStateUpdated.emit(True)
                return
    ...
    @QtCore.Slot()
    def _scan_odmr_line_qudi(self):
        """Perform a single scan over the specified frequency range test
        dear mr qudi, please work.
        """
        with self._threadlock:
            ...
            try:
                ...
                self.log.exception(
                    "Error while trying to read ODMR scan data from hardware:"
                )
                self.stop_odmr_scan()
                return
            ...
            if self._elapsed_sweeps == current_line_buffer_size:
                self.log.debug(
                    f"extending data grid for sweep number {self._elapsed_sweeps}"
                )
                expand_arrays = tuple(
                    np.full((r[-1], self.__estimated_lines), np.nan)
                    for r in self._scan_frequency_ranges
                )
                self._raw_data = {
                    ch: [
                        np.concatenate((r, expand_arrays[ii]), axis=0)
                        for ii, r in enumerate(range_list)
                    ]
            ...
            for ch, range_list in self._raw_data.items():
                ...
                for range_index, range_params in enumerate(self._scan_frequency_ranges):
                    range_list[range_index] = np.roll(
                        range_list[range_index], 1, axis=1
                    )
    ...
    def _scan_odmr_line(self):
        ...
        written by SHG, reinstated into use 7/18
        """
        with self._threadlock:
            # If the odmr measurement is not running do nothing and break the Qt signal loop
            if self.module_state() != "locked":
            ...
            try:
                ...
                ##
                # init of self._raw_linedata at freq_arr_index
                # scanner.start_buffered_acquisition()
                # time.sleep(0.1)
                for step in range(
                    len(np.concatenate(self._frequency_data))
                ):  # (len(microwave._scan_frequencies)):
                    ##time.sleep(1/microwave._scan_sample_rate) # this will be replaced by the acquisistion
                    new_data_oversamp = scanner.acquire_frame()
                    #
                    # new_data_oversamp = scanner.get_buffered_samples(self._oversampling_factor)
                    # new_counts = scanner.acquire_frame()
                    for ch in new_data_oversamp:  # new_counts:
                        self._raw_linedata[ch][step] = np.mean(new_data_oversamp[ch])
                    ##print('sending trigger')
                    self._microwave()._device.write("*TRG")
                    time.sleep(0.008)  # Time to set the new freq to the SRS
                # scanner.stop_buffered_acquisition()
                #
                new_counts = self._raw_linedata
                self._microwave().reset_scan()
    ...
    def _get_metadata(self):
        metadata = {
            ...
            "Start Frequencies (Hz)": tuple(
                rng[0] for rng in self._scan_frequency_ranges
            ),
            "Stop Frequencies (Hz)": tuple(
                rng[1] for rng in self._scan_frequency_ranges
            ),
        ...
        for fit_channel in self._fit_results:
            for ii, fit_result in enumerate(self._fit_results[fit_channel]):
                if fit_result:
                    ...
                    metadata[f'fit result (channel "{fit_channel}" range {ii})'] = (
                        export_dict
                    )
        return metadata
    ...
    def _join_channel_raw_data(self, channel):
        ...
        joined_data = np.concatenate(
            [raw[:, : self._elapsed_sweeps] for raw in channel_data], axis=0
        )
    ...
    def save_odmr_data(self, tag=None):
        ...
        with self._threadlock:
            ...
            data_storage = TextDataStorage(
                root_dir=self.module_default_data_dir, column_formats=".15e"
            )
            for channel, range_data in self._raw_data.items():
    ...
    def _draw_figure(self, channel, range_index):
        ...
        # plot raw data
        raw_data_plot = ax_raw.imshow(
            raw_data.transpose(),
            cmap=plt.get_cmap("inferno"),
            origin="lower",
            vmin=np.min(raw_data),
            vmax=np.max(raw_data),
            extent=[min(freq_data), max(freq_data), -0.5, raw_data.shape[1] - 0.5],
            aspect="auto",
